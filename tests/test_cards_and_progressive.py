...
def test_simple_and_standard_cards_serialize() -> None:
    ...
    standard = ResponseBuilder().with_standard_card(
        "Recording",
        "A short description",
    ...
    invalid = ResponseBuilder().with_standard_card(
...
def _handler_input(request_type: str = "IntentRequest"):
    ...
    return SimpleNamespace(
        ...
        attributes_manager=SimpleNamespace(
            get_request_attributes=lambda: request_attributes,
        ),
    )
...
async def test_progressive_response_excludes_audio_player_and_is_best_effort() -> None:
    ...
    assert await audio_client.send(
        _handler_input("AudioPlayer.PlaybackNearlyFinished"), "One moment.",
    ) is False
