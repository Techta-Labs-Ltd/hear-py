...
def test_normalizes_current_alexa_search_contract():
    ...
    raw = {
        ...
        "shortDescription": "Monthly roundup of extra features from the Shetland Times newspaper.",
        "creator": {"id": "creator-1", "name": "Shetland Life"},
        "organization": {"id": "organization-1", "name": "Shetland Life"},
...
def test_content_id_without_audio_is_still_rejected():
    assert ContentNormalizer.normalize_content_items([{"contentId": "content-1"}]) == []
...
def test_publication_tracks_are_flattened_with_parent_metadata():
        ...
        "tracks": [
            {
                ...
                "durationSecs": 120,
                "playbackSpeed": [
                    {"speed": 1.5, "audioUrl": "https://cdn.hear.media/track-1-15.mp3"}
                ],
            },
            {
                "contentId": "track-2",
    ...
    renormalized = ContentNormalizer.normalize_content_items(items)
