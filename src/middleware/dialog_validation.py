...
class DialogValidationPolicy:
    ...
    _SOURCE_NAME_PROMPTS = {
        ...
        "organization_name": (
            ...
            "Say the talking newspaper's full name. For example, Tynedale Talking Newspaper.",
        ),
        "publication_source": (
    ...
    def dialog_validation_failure(handler_input) -> dict | None:
        ...
        if (
            ...
            and not ConfirmationPolicy.allows_search_replacement(
                dialog_type,
                intent_name,
                ConfirmationPolicy.raw_utterance(handler_input),
            )
        ):
            speech, reprompt = DialogValidationPolicy._binary_prompt(active)
        elif (
