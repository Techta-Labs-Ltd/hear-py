...
class LambdaDeadlineInterceptor(AbstractRequestInterceptor):
    ...
    async def process(self, handler_input: HandlerInput):
        ...
        print("Hear: request budget requestType=%s remainingMs=%s hasLambdaContext=%s"
        ...
        if isinstance(remaining_ms, (int, float)) and remaining_ms > 0 and remaining_ms < 6000:
            print("Hear: Lambda timeout is below 8s; search may fail. "
                  "Set function timeout to 8 seconds. remainingMs=%s" % remaining_ms)
