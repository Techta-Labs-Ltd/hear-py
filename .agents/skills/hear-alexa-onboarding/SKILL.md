...
2. **Registered-user + Amazon API city detection.** Check the user is known and, when device permissions are granted, detect the city from the Amazon API before asking. The primitives live in `src/services/alexa/locality.py` (`has_permission`, `get_geolocation`, `get_device_address`) and the scopes in `config/permission_scopes.py`. If the user declines permission or the API returns nothing, fall back to manual town capture.
3. **Permission ask.** `ask_for_permission` (stage `ask_permission`) → yes sends an `AskForPermissionsConsent` card for DEVICE_ADDRESS + GEOLOCATION; no falls back to manual town entry (`ONBOARDING_LOCATION_DENIED`, stage `ask_town`). A granted permission must not re-trigger the ask.
